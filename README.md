...
Rust: Version 1.65 or higher (install via rustup).
Docker: Installed and running with user permissions (see Docker installation).
...
--hot-fix
Skip health checks for urgent deployments
false
...
--port
Internal container port (maps to external 3000)
80


--health-check-timeout
Timeout per health check attempt in seconds
5
...
Add the Binary:Copy target/release/container-test-tool to your repository or build it in your workflow.
...
Prepare a Dockerfile:Ensure your Dockerfile builds an application with a health check endpoint (e.g., FastAPI server with /health).
Example main.py:
...
Success Case:cargo run --release -- --dockerfile-path /path/to/Dockerfile --port 8000 --health-check-path /health

Expect green success messages and cleanup.
Failure Case:cargo run --release -- --dockerfile-path /path/to/Dockerfile --port 8000 --health-check-path /invalid

Expect red failure messages with logs.
Hot-Fix Case:cargo run --release -- --dockerfile-path /path/to/Dockerfile --hot-fix
...
Built with ❤️ for reliable CI/CD pipelines.
