...
async fn main() -> Result<()> {
    ...
    let (id, c_name, i_name) = match test_docker_container(&cli).await {
        ...
        Err(_) => {
            ...
            return Err(anyhow!("Container failed to become healthy"));
        }
    };
    let image_id = id;
...
async fn cleanup_docker(image_id: &str, container_name: &str, _image_name: &str) -> Result<()> {
        ...
        if image_exists {
            ...
            if name_exists {
                ...
                let output = Command::new("docker")
                    ...
                    .output()
                    .await
                    .context(format!("Failed to remove image by name: {}", _image_name))?;
                println!(
                    "{}",
                    Colour::Yellow.paint(format!(
    ...
    let dangling_output_str = String::from_utf8_lossy(&dangling_output.stdout);
    let dangling_image_ids = dangling_output_str
        .trim()
        .lines()
        .collect::<Vec<_>>();
